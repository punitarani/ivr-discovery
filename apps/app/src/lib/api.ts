...
export type DiscoverResponse = { status: string; sessionId: string }

export async function postDiscover(phone: string): Promise<DiscoverResponse> {
  ...
  return (await res.json().catch(() => ({}))) as DiscoverResponse
}
